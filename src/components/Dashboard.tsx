    ...
    const fetchAiInsight = async () => {
      ...
      try {
        ...
        const userContext = user ? `
          Öğrenci Bilgileri:
          Alan: ${user.major || 'Belirtilmedi'}
          Hedef Sıralama: ${user.targetRank || 'Belirtilmedi'}
          Hedef Bölüm: ${user.targetGoal || 'Belirtilmedi'}
          Mezuniyet: ${user.educationStatus || 'Belirtilmedi'}
          Temel Seviye: ${user.baseLevel ? JSON.stringify(user.baseLevel) : 'Belirtilmedi'}
          Günlük Çalışma Hedefi: ${user.dailyStudyHours ? user.dailyStudyHours + ' saat' : 'Belirtilmedi'}
          En Çok Zorlandığı Ders: ${user.hardestSubject || 'Belirtilmedi'}
        ` : '';
        ...
        const response = await ai.models.generateContent({
          ...
          contents: [{
            role: "user",
            parts: [{ text: `${userContext}\n\nYukarıdaki bilgilere ve bugünkü çalışmalarına (paragraf, trigonometri, fizik, problem, biyoloji, kimya) göre öğrenciye kısa, motive edici, hedefine odaklı ve tek cümlelik bir tavsiye ver.` }]
          }],
        });
        setAiInsight(response.text || "Bugün harika gidiyorsun, tempoyu koru!");
      } catch (error) {
        setAiInsight("Bugün planlanan hedeflerine odaklanman netlerini artıracaktır. Adım adım ilerle!");
      } finally {
    ...
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-3 sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-lg font-bold leading-tight tracking-tight">UVA Mentor</h2>
        </div>
      ...
      <div className="flex flex-1">
        ...
        <aside className="hidden lg:flex w-72 flex-col bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-6 gap-8">
          <div className="flex flex-col gap-2">
            ...
            <nav className="flex flex-col gap-1">
              ...
              <SidebarItem icon={Ticket} label="Destek Sistemi" onClick={() => onNavigate('support')} />
              {user?.role === 'admin' && (
                <SidebarItem icon={ShieldCheck} label="Yetkili Paneli" onClick={() => onNavigate('admin')} />
              )}
            </nav>
          </div>
          <div className="mt-auto p-4 rounded-xl bg-primary/5 border border-primary/10 flex flex-col gap-3">
            <p className="text-sm font-bold text-primary">UVA Mentor Premium</p>
        ...
        <main className="flex-1 p-6 md:p-10 overflow-y-auto max-w-7xl mx-auto w-full">
          <div className="flex flex-col gap-8">
            ...
            >
              <h1 className="text-3xl font-black tracking-tight">Merhaba, {user?.name.split(' ')[0] || 'Öğrenci'}! 👋</h1>
            ...
            >
              <div className="flex gap-4 items-start">
                ...
                <div className="flex flex-col gap-1">
                  <h3 className="font-bold text-primary">UVA Mentor Görüşü</h3>
                  {isAiLoading ? (
