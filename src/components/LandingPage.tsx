...
const FAQItem = ({ question, answer }: { question: string, answer: string }) => {
  ...
  return (
    <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden bg-white dark:bg-slate-900 transition-all hover:border-primary/30">
      <button
      ...
      >
        <div className="px-6 pb-6 text-slate-500 dark:text-slate-400 text-sm leading-relaxed border-t border-slate-50 dark:border-slate-800 pt-4">
          {answer}
        </div>
      </motion.div>
    </div>
...
interface LandingPageProps {
  onNavigate: (view: 'landing' | 'about' | 'pricing' | 'login' | 'register') => void;
  onLogin: () => void;
  onRegister: () => void;
}
      ...
      <div className="layout-container flex h-full grow flex-col">
        ...
        <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-200 dark:border-slate-800 px-6 md:px-20 py-4 bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md sticky top-0 z-50">
          <div className="flex items-center gap-2 text-primary">
            ...
            <h2 className="text-slate-900 dark:text-slate-100 text-xl font-bold leading-tight tracking-tight">UVA Mentor</h2>
          </div>
          ...
          <nav className="hidden md:flex flex-1 justify-center gap-8">
            ...
            <a className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors" href="#sss">SSS</a>
            <button onClick={() => onNavigate('pricing')} className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors">Fiyatlandırma</button>
            <button onClick={() => onNavigate('about')} className="text-slate-600 dark:text-slate-400 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors">Hakkımızda</button>
          </nav>
        ...
        {isMenuOpen && (
          ...
          >
            <a className="text-lg font-medium text-left" href="#ozellikler" onClick={() => setIsMenuOpen(false)}>Özellikler</a>
            <a className="text-lg font-medium text-left" href="#sss" onClick={() => setIsMenuOpen(false)}>Sıkça Sorulan Sorular</a>
            <button className="text-lg font-medium text-left" onClick={() => { onNavigate('pricing'); setIsMenuOpen(false); }}>Fiyatlandırma</button>
            <button className="text-lg font-medium text-left" onClick={() => { onNavigate('about'); setIsMenuOpen(false); }}>Hakkımızda</button>
        ...
        <main className="flex flex-col flex-1">
                  ...
                  <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold">UVA Mentor</span>
                      </div>
          ...
          {/* FAQ Section */}
          <section className="px-6 md:px-20 py-24 bg-white dark:bg-slate-900/50" id="sss">
            <div className="max-w-4xl mx-auto">
              <div className="text-center mb-16">
                <h2 className="text-primary font-bold tracking-widest uppercase text-sm mb-4">Sıkça Sorulan Sorular</h2>
                <h3 className="text-slate-900 dark:text-slate-100 text-3xl md:text-5xl font-extrabold tracking-tight mb-6">Aklınıza takılan her şeyi yanıtladık</h3>
                <p className="text-slate-500 text-lg">Eğer aradığınız cevabı bulamazsanız, bize her zaman destek panelinden ulaşabilirsiniz.</p>
              </div>

              <div className="space-y-4">
                {[
                  {
                    q: "1. UVA Mentor nedir ve klasik ders programlarından farkı nedir?",
                    a: "UVA Mentor, standart ve \"herkese aynı\" listeler veren bir program değildir. Sizi önce bir Seviye Tespit sınavına tabi tutar. Yapay zeka algoritmamız, yanlış yaptığınız sorulardan yola çıkarak \"öğrenme eksikliğinizi\" tespit eder ve size her gün neyi, ne kadar çalışmanız gerektiğini söyleyen dinamik bir reçete yazar."
                  },
                  {
                    q: "2. \"AI Reçete\" sistemi nasıl çalışıyor?",
                    a: "Tıpkı bir doktorun tahlil sonuçlarınıza göre ilaç yazması gibi; AI Reçete de sizin çözdüğünüz sorulardaki hata payınızı, hızınızı ve konu hakimiyetinizi analiz eder. Eğer bir konuda zayıfsanız, sistem o konuyu programınıza daha sık dahil eder; uzmanlaştığınız konularda ise sadece \"hatırlatma testleri\" vererek zaman kazanmanızı sağlar."
                  },
                  {
                    q: "3. Programım aksarsa veya bir gün çalışamazsam ne olur?",
                    a: "Sistemimizin en güçlü yanlarından biri esnekliğidir. Eğer bir gün görevlerinizi tamamlayamazsanız, yapay zeka bu eksikliği sonraki günlere \"en verimli şekilde\" yayar. Programınız asla \"çöp\" olmaz; UVA Mentor rotayı hedefinize göre anında yeniden hesaplar."
                  },
                  {
                    q: "4. \"Yetişiyor mu?\" göstergesi neye göre hesaplanıyor?",
                    a: "Bu gösterge, mevcut çalışma temponuz, konu tamamlama hızınız ve sınav tarihine kalan sürenin matematiksel bir simülasyonudur. Eğer mevcut hızınızla hedeflediğiniz sıralama/bölüm risk altındaysa sistem sizi turuncu/kırmızı uyarılarla uyarır ve tempoyu artırmanız için rehberlik eder."
                  },
                  {
                    q: "5. Elimizdeki soru bankalarını sisteme dahil edebilir miyiz?",
                    a: "Evet! \"Kitaplarım\" sekmesinden elinizdeki yayınları seçebilirsiniz. UVA Mentor, size ödev verirken piyasadaki popüler kaynaklarla entegre çalışır. Böylece \"hangi kitaptan çözsem?\" diye düşünmezsiniz; AI size elinizdeki kitabın hangi sayfasını çözmeniz gerektiğini söyler."
                  },
                  {
                    q: "6. Kimler Kullanabilir?",
                    a: "UVA Mentor için yaşın, mevcut durumun veya hangi aşamada olduğunun bir önemi yok. Bizim için tek bir kriter var: YKS’de başarıya ulaşma iradesi. Sistemimiz; Lise Öğrencileri İçin (Okul dersleri ile sınav maratonunu hatasız bir zaman yönetimiyle birleştirmek isteyenlere), Mezunlar İçin (Geçmiş hatalarını analiz edip bu yıl \"nokta atışı\" bir stratejiyle hedefe kilitlenenlere), Üniversite Öğrencileri veya Çalışanlar İçin (Hayallerini ertelemeyip, kısıtlı zamanını yapay zeka verimliliğiyle yöneterek yeniden başlamak isteyenlere), Temeli Olmayanlar İçin (En doğru başlangıç noktasını belirleyerek rehberlik eder)."
                  },
                  {
                    q: "7. Deneme netlerimi sisteme girebilir miyim?",
                    a: "Kesinlikle! Girdiğiniz her TYT ve AYT deneme sonucu, yapay zekanın sizi daha iyi tanımasını sağlar. Netlerinizdeki dalgalanmaları analiz eden UVA Mentor, moraliniz bozulmadan rotarınızı günceller ve hangi branşa ağırlık vermeniz gerektiğini anlık olarak değiştirir."
                  },
                  {
                    q: "8. Farklı seviyelerdeki öğrenciler aynı verimi alabilir mi?",
                    a: "Evet. Sıfırdan başlayan bir öğrenci için sistem \"temel oluşturma\" modunda çalışırken, derece hedefleyen bir öğrenci için \"detay ve hız\" moduna geçer. Yapay zeka, sizin seviyenize göre soruların ve konuların dozajını otomatik olarak ayarlar."
                  },
                  {
                    q: "9. Uygulama sadece sayısal dersler için mi geçerli?",
                    a: "Hayır. YKS kapsamındaki tüm dersler (Türkçe, Matematik, Fen Bilimleri, Sosyal Bilimler ve Dil) sistemimize dahildir. Sözel derslerde \"hatırlatma ve tekrar\" döngülerini, sayısal derslerde ise \"problem çözme ve mantık yürütme\" süreçlerini ön plana çıkaran farklı algoritmalar kullanılır."
                  },
                  {
                    q: "10. Verilerim ve çalışma geçmişim güvende mi?",
                    a: "Kişisel verileriniz ve akademik gelişim tarihçeniz en yüksek güvenlik standartlarıyla korunur. İlerleme verileriniz sadece size özel analizler oluşturmak ve başarınızı takip etmek için kullanılır."
                  },
                  {
                    q: "11. UVA Mentor'u diğer online eğitim platformlarından ayıran en büyük fark nedir?",
                    a: "Diğer platformlar genellikle size \"içerik\" (video, soru vb.) sunar. UVA Mentor ise size \"strateji\" sunar. Biz içerik yığını içinde boğulmanızı değil, o içeriklerden hangisinin sizin eksikliğinizi kapatacağını söyleyen akıllı bir katman sunuyoruz."
                  }
                ].map((faq, idx) => (
          ...
          <section className="px-6 md:px-20 py-24 bg-primary text-white overflow-hidden relative">
            ...
            <div className="max-w-5xl mx-auto text-center relative z-10">
              ...
              <p className="text-white/80 text-lg md:text-xl mb-12 max-w-2xl mx-auto">Binlerce öğrenci UVA Mentor ile netlerini artırıyor. Sen de bu başarının bir parçası olmaya hazır mısın?</p>
        ...
        <footer className="px-6 md:px-20 py-16 bg-background-light dark:bg-background-dark border-t border-slate-200 dark:border-slate-800">
          <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-12">
            <div className="col-span-1 md:col-span-1">
              <div className="flex items-center gap-2 text-primary mb-6">
                ...
                <h2 className="text-slate-900 dark:text-slate-100 text-lg font-bold">UVA Mentor</h2>
              </div>
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-slate-500">
                <li><a className="hover:text-primary transition-colors" href="#ozellikler">Özellikler</a></li>
                <li><a className="hover:text-primary transition-colors" href="#sss">Sıkça Sorulan Sorular</a></li>
                ...
                <li><button className="hover:text-primary transition-colors" onClick={() => onNavigate('pricing')}>Fiyatlandırma</button></li>
              </ul>
            </div>
            <div>
              <h5 className="font-bold text-slate-900 dark:text-slate-100 mb-6">Şirket</h5>
              <ul className="space-y-4 text-sm text-slate-500">
                <li><button className="hover:text-primary transition-colors" onClick={() => onNavigate('about')}>Hakkımızda</button></li>
          ...
          <div className="max-w-7xl mx-auto mt-16 pt-8 border-t border-slate-200 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-slate-500 text-sm">© 2024 UVA Mentor. Tüm hakları saklıdır.</p>
